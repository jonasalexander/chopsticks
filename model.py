...
class Model:
    def __init__(self, attack_dict=None, redistribution_dict=None, debug=False):
        self.debug = debug
    ...
    def __str__(self):
        return "Attack dict: {0} \nRedistribute dict: {1}".format(
            self._attack_dict, self._redistribution_dict
        )
    ...
    def attack(self, state):
        ...
        if state not in self._attack_dict:
            if num_hands == 4:
                ...
                if len(possible_indices) == 2:
                    ...
                    if possible_indices[1] > 1:
                        ...
                        if option == 0:
                            ...
                            raise ValueError(
                                "Receive unexpected state-options matchup in Model.attack(): state - %s, options - %s"
                                % (state, self._attack_dict[state])
                            )
                    else:
                    ...
                    raise ValueError(
                        "Received unexpected length of possible_indices: %s"
                        % possible_indices
                    )
        ...
        raise ValueError(
            "Got a random choice %s outside the limit of options in self._attack_dict[state] %s with state %s."
            % (rorig, self._attack_dict[state], state)
        )

    def redistribute(self, state):
        # can only do it if one hand has 0 fingers
        self_state = state[0:2]
        if 0 not in self_state:
            return (False, False)
