...
class ComputerPlayer(Player):
    def __init__(self, model=None, name=None, debug=False):
        super(ComputerPlayer, self).__init__(name=name, debug=debug)
    ...
    def turn(self, opponent):
        state = tuple([i.num for i in self.hands] + [i.num for i in opponent.hands])
        ...
        if self.debug:
            ...
            print(
                "Move was to add hand %s to hand %s"
...
class InteractivePlayer(Player):
    ...
    def turn(self, opponent):
        ...
        if 0 in self_state and all([i % 2 == 0 for i in self_state]):
            redistribute = input(
                "Do you want to redistribute your fingers?(0 for no and 1 for yes) \n--->"
            )
            if redistribute == "1":
                for i, h in enumerate(self.hands):
                    if h.num == 0:
                ...
                print(opponent)
                state = tuple(
                    [i.num for i in self.hands] + [i.num for i in opponent.hands]
                )
                self_state = state[0:2]
                opponent_state = state[2:4]
            elif redistribute == "0":
                ...
                print(
                    "Didn't get a 0 or 1 as expected for redistribution quesiton. Exiting"
                )
                return False
        ...
        if 0 in self_state or self_state[0] == self_state[1]:
            ...
            self_choice_str = input(
                "Which of your hands would you like to use to attack? (0-indexed) \n--->"
            )
            try:
                ...
                print(
                    "Didn't get a 0 or 1 as expected for self_choice quesiton. Exiting"
                )
                return False
        ...
        if 0 in opponent_state or opponent_state[0] == opponent_state[1]:
            ...
            opponent_choice_str = input(
                "Which of your opponent's hands would you like to attack? (0-indexed) \n--->"
            )
            try:
                ...
                print(
                    "Didn't get a 0 or 1 as expected for opponent_choice question. Exiting"
                )
                return False
